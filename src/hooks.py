...
def process_self_attention(output, batch_size, num_tokens, num_attn_heads, embed_dim, scale, num_global_tokens, ret_self_attn_maps=False):
    ...
    q, k, v = qkv[0] * scale, qkv[1], qkv[2]  # [64, 16, 1029, 48]
    attn = q @ k.transpose(-2, -1)  # [64, 16, 1029, 1029]
    self_attn_maps = attn[:, : , 0, num_global_tokens:]  # [64, 16, 1024] 
    self_attn = self_attn_maps.mean(dim=1)  # [64, 1024]
    self_attn = self_attn.softmax(dim=-1)  # [64, 1024]
