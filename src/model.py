...
class DINOJSCC_Adapter(nn.Module):
    """
    Creates a jscc encoder on top of the DINO encoder.
    ...
    def __init__(
        ...
        self.text_adapter = nn.Sequential(
            nn.Linear(clip_embed_dim, dino_embed_dim),
            act,
            nn.Linear(dino_embed_dim, dino_embed_dim),
        )
        if text_adapter_pretrain != "":
            saved_state_dict = torch.load(text_adapter_pretrain)

            new_state_dict = {}
            key_mapping = {
                "linear_layer.weight": "0.weight",
                "linear_layer.bias": "0.bias",
                "hidden_layers.0.weight": "2.weight",
                "hidden_layers.0.bias": "2.bias",
            }

            for key in saved_state_dict:
                if key in key_mapping:
                    new_state_dict[key_mapping[key]] = saved_state_dict[key]

            self.text_adapter.load_state_dict(new_state_dict)
            print("Text adapter weights loaded from: ", text_adapter_pretrain)

        for param in self.text_adapter.parameters():
            param.requires_grad = False

        self.visual_adapter = ConvModule(  # Conv(dino_embed_dim, k, kernel_size=(1, 1), stride=(1, 1),), BatchNorm2d, GELU
            dino_embed_dim,
            self.k,
            1,
        ...
        self.semantic_restorer = (
            nn.Sequential(  # k->k, k->dino_embed_dim, dino_embed_dim->dino_embed_dim
                ConvModule(
                    self.k,
                    self.k,
                    1,
        ...
        self.alignment_strategy = (
            alignment_strategy  # relevant only if we use disentangled_self_attn
        )
    ...
    def from_config(cls, config):
        ...
        model = cls(
            ...
            text_adapter_pretrain=config.get("text_adapter_pretrain", ""),
            k=config.get("k", 256),
            act_cfg=config.get("act_cfg", dict(type="GELU")),
            norm_cfg=config.get("norm_cfg", dict(type="BN")),
            conv_cfg=config.get("conv_cfg", None),
        )
        return model
    ...
    def forward(
        ...
        textual_embedding = self.text_adapter(textual_embedding)
        ori_visual_embedding = visual_embedding.clone()
        visual_embedding = self.visual_jscc(visual_embedding)
        rec_visual_embedding = visual_embedding.clone()
        ...
        if not return_index:
            x = self.compute_similarity(
                visual_embedding, textual_embedding, text_input_mask, return_index
            )
        else:
            x, index = self.compute_similarity(
                visual_embedding, textual_embedding, text_input_mask, return_index
            )
        ...
        if not return_index:
            return x, ori_visual_embedding, rec_visual_embedding
        else:
            return x, index

    def visual_jscc(self, visual_embedding, fix_snr=False, channel_type="awgn"):
        b, np, c = visual_embedding.shape
        nh = nw = int(np ** (1 / 2))
        visual_embedding = visual_embedding.permute(0, 2, 1).reshape(b, c, nh, nw)
        # visual_embedding = F.interpolate(
        #     visual_embedding.float().unsqueeze(0),
        #     (np, self.k),
        #     mode="bilinear",
        #     align_corners=True,
        # )[0]
        visual_embedding = self.visual_adapter(visual_embedding)
        visual_embedding = self.thru_channel(
            visual_embedding, fix_snr=fix_snr, channel_type=channel_type
        )
        # visual_embedding = visual_embedding.permute(0, 2, 1).reshape(b, self.k, nh, nw)
        visual_embedding = self.semantic_restorer(visual_embedding)
        visual_embedding = visual_embedding.reshape(b, c, -1).permute(0, 2, 1)

        return visual_embedding

    def thru_channel(self, x, fix_snr=False, channel_type="awgn"):
        if fix_snr:
            snr = fix_snr
        else:
            # SNR随机从1-20中生成，根据SNR生成和x相同形状的噪声来模拟AWGN
        ...
        if channel_type == "awgn":
            ...
            x_dim = x.shape[1] // 2
            x_in = torch.complex(x[:, :x_dim], x[:, x_dim:])

            signal_power = torch.mean(torch.abs(x_in) ** 2)
            noise_power = signal_power / (10 ** (snr / 10))
            noise_std = torch.sqrt(2 * noise_power)
            noise_real = torch.normal(size=x_in.shape, mean=0.0, std=noise_std.item())
            noise_imag = torch.normal(size=x_in.shape, mean=0.0, std=noise_std.item())
            noise = noise_real + 1j * noise_imag
            h = torch.sqrt(
                torch.normal(size=x_in.shape, mean=0.0, std=1) ** 2
                + torch.normal(size=x_in.shape, mean=0.0, std=1) ** 2
            ...
            z_out = torch.concat([torch.real(z_out), torch.imag(z_out)], 1)
    ...
    def compute_similarity(
        ...
        if len(visual_embedding.shape) == 3 or len(textual_embedding.shape) == 3:
            ...
            if self.alignment_strategy == "weighted_avg":
                if (
                    len(visual_embedding.shape) != 3
                    or len(textual_embedding.shape) != 2
                ):
                    raise Exception(
                        "Alignment strategy not implemented for this type of embeddings!"
                    )
                ...
                visual_embedding = (visual_embedding * sims.unsqueeze(dim=-1)).mean(
                    dim=1
                )
            ...
            # in this case we sample the visual embedding from the softmax similarities of attention heads tokens and the textual tokens
                ...
                index = (
                    torch.multinomial(sims, 1)
                    .view(-1, 1, 1)
                    .expand(-1, 1, visual_embedding.shape[-1])
                )
                ...
                visual_embedding = torch.gather(
                    visual_embedding, 1, index_reshaped
                ).squeeze(1)
                sims = textual_embedding @ visual_embedding.transpose(1, 0)
            else:
                ...
                if textual_embedding.shape[1] > 1:
                    assert (
                        text_input_mask is not None
                    ), "If we use all the textual embeddings, we need the input mask"
                ...
                im_set = im_set.unsqueeze(1).expand(
                    -1, s_seq_batch, -1, -1
                )  # B x B x S_im x dim
                s_seq = s_seq.unsqueeze(0).expand(
                    im_set_batch, -1, -1, -1
                )  # B x B x S_s x dim
                alignments = torch.matmul(
                    im_set, s_seq.permute(0, 1, 3, 2)
                )  # B x B x S_im x S_s

                # compute mask for the alignments tensor
                if text_input_mask is not None:
                    alignment_mask = (
                        text_input_mask.unsqueeze(1)
                        .unsqueeze(0)
                        .expand(im_set_batch, -1, im_set_len, -1)
                        .logical_not()
                    )
                ...
                if self.alignment_strategy == "sum":
                    ...
                    mins = (
                        sorted_alignments.min(2)[0]
                        .unsqueeze(-1)
                        .expand(-1, -1, s_seq_len)
                    )
                    maxs = (
                        sorted_alignments.max(2)[0]
                        .unsqueeze(-1)
                        .expand(-1, -1, s_seq_len)
                    )
...
class VisualProjectionLayer(nn.Module):
    ...
    def from_config(cls, config):
        ...
        model = cls(
            ...
            cosine=config.get("cosine", True),
            hidden_embed_dim=(
                config.get("hidden_embed_dim", None)
                if config.get("hidden_layer", False)
                else None
            ),
            dino_embed_dim=config.get("dino_embed_dim", 1024),
            clip_embed_dim=config.get("clip_embed_dim", 512),
...
class ProjectionLayer(nn.Module):
    ...
    def __init__(
        ...
        if hidden_layer:
            hidden_layer = (
                1 if hidden_layer is True else hidden_layer
            )  # ensuring compatibility with old code
    ...
    def forward(
        ...
        if self.weight_attn_heads is not None:
            assert (
                self_attn_maps is not None
            ), "In case we have attention maps weights, we have to weight patch tokens mean by the weighted self-attention maps"
            visual_embedding = self.get_visual_embed(
                visual_embedding, self_attn_maps=self_attn_maps, cls=cls
            )
    ...
    def get_visual_embed(self, visual_embedding, self_attn_maps=None, cls=None):
        if self_attn_maps is not None:
            ...
            assert (
                len(visual_embedding.shape) == 3
            ), "In case we have attention maps weights, the visual_embedding should contain patch embeddings, with shape BS x NUM_PATCHES x EMBED_DIM"
            if self.weight_attn_heads == "conditioned":
                assert (
                    cls is not None
                ), "cls must be setted in case of dinamic attention weighting"
                ...
                self_attn = (
                    self_attn_maps
                    * normalized_attn_weights.view(
                        1, normalized_attn_weights.shape[0], 1
                    )
                ).mean(dim=1)
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        if "linear_layer2.weight" in state_dict:
            state_dict["hidden_layers.0.weight"] = state_dict.pop(
...
class DoubleMLP(nn.Module):
    def __init__(
        ...
        if hidden_layer:
            ...
            self.visual_hidden_layers = nn.ModuleList(
                [nn.Linear(dino_embed_dim, dino_embed_dim) for _ in range(hidden_layer)]
            )
    ...
    def compute_similarity(
        self, visual_embedding, textual_embedding, text_input_mask=None
    ):
        if len(visual_embedding.shape) == 3 or len(textual_embedding.shape) == 3:
            # at least one embedding is decomposed: either we have all textual tokens or we have all the attention head tokens
    ...
    def forward(
        ...
        x = self.compute_similarity(
            visual_embedding, textual_embedding, text_input_mask
        )
        if not ret_similarity_matrix:
            x = x[torch.eye(len(x)) > 0.5]  # only diagonal elements
...
class CLIPLastLayer(nn.Module):
    def __init__(
        ...
        self.projection_layer = ProjectionLayer(
            ...
            weight_attn_heads=weight_attn_heads,
    ...
    def forward(
        ...
        if ret_embeds:
            textual_embedding, visual_embedding = self.projection_layer(
                visual_embedding,
                x,
...
class DinoText(nn.Module):
    ...
    def __init__(
        self,
        dino_cfg="dinov2_vitl14_reg",
        clip_cfg="ViT-B/16",
        projection_cfg="configs/linear.yaml",
        projection_weights="weights/linear_avg_self_attn_out.pth",
        freeze_text_encoder=True,
        avg_self_attn_token=True,
        use_disentangled_self_attn=False,
    ):
        super().__init__()
        # DINO parameters
        ...
        if self.use_avg_self_attn or self.use_disentangled_self_attn:
            self.visual_backbone.blocks[-1].attn.qkv.register_forward_hook(
                get_self_attention
            )
    ...
    def encode_image(self, images):
        ...
        x = self.visual_backbone(
            images,
            is_training=self.avg_self_attn_token or self.use_disentangled_self_attn,
        )
        if self.avg_self_attn_token:
            ...
            self_attn = self.process_self_attention(
        ...
        if self.use_disentangled_self_attn:
            ...
            x = (
                x["x_norm_patchtokens"].unsqueeze(1) * self_attn_maps.unsqueeze(-1)
            ).mean(dim=2)
        return x
    ...
    def process_self_attention(
        ...
        qkv = output.reshape(
            batch_size, num_tokens, 3, num_attn_heads, embed_dim // num_attn_heads
        ).permute(2, 0, 3, 1, 4)
    ...
    def __len__(self):
        ...
        return (
            count_parameters(self.visual_backbone)
            + count_parameters(self.clip2dino_proj)
            + count_parameters(self.text_backbone.transformer)
        )
