...
class DinoClipDataset(Dataset):
    def __init__(
        ...
        if is_wds:
            self.__load_wds_dataset(
                features_file, features_name, text_features, load_attn_maps
            )
        else:
            self.__load_pth_dataset(
                features_file, features_name, text_features, load_attn_maps
            )

    def __getitem__(self, idx):
        annotation = self.data[idx]["annotation"]
        if self.features_name == "patch_tokens":
            image = torch.from_numpy(np.load(self.data[idx]["image"]))
        else:
    ...
    def __load_pth_dataset(
        ...
        for idx, ann in enumerate(data["annotations"]):
            ...
            if text_features != "clip_txt_out_tokens_avg":
                ...
                self.data[idx]["annotation"] = ann["clip_txt_out_tokens"][mask].mean(
                    dim=0
                )
            if text_features == "clip_second_last_out":
                self.data[idx]["text_argmax"] = ann["text_argmax"]
            if features_name in images[imm_id].keys():
                self.data[idx]["image"] = images[imm_id][features_name]
            else:
                features_dir = os.path.join(
                    os.path.dirname(features_file), features_name
                )
                self.data[idx]["image"] = os.path.join(features_dir, f"{imm_id}.npy")
            if load_attn_maps:
    ...
    def __load_wds_dataset(
        ...
        for idx, obj in enumerate(dataset):
            ...
            if text_features != "clip_txt_out_tokens_avg":
                ...
                mask[mask.sum() - 1] = False  # excluding end of sequence
                mask[0] = False  # excluding CLS token
                self.data[idx]["annotation"] = obj["pth"]["clip_txt_out_tokens"][
                    mask
                ].mean(dim=0)
