...
class DINOText(nn.Module):
    ...
    def get_clip_second_last_dense_out(
        self, model: torch.nn.Module, input: torch.Tensor, output: torch.Tensor
    ):
        self.feats["clip_second_last_out"] = output
        self.feats["clip_second_last_out"].to(dtype=torch.float32)

    def get_all_out_tokens(
        self, model: torch.nn.Module, input: torch.Tensor, output: torch.Tensor
    ):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        mean = (
            (0.485, 0.456, 0.406)
            if not "clip" in model_name
            else (0.4815, 0.4578, 0.4082)
        )
        std = (
            (0.229, 0.224, 0.225)
            if not "clip" in model_name
            else (0.2686, 0.2613, 0.2758)
        )
        ...
        if "bert" in self.clip_model_name:
            self.clip_model = BertModel.from_pretrained(
                self.clip_model_name, output_hidden_states=False
            )
        ...
        with open(
            os.path.join("configs/second_stage", f"{proj_class}.yaml"), "r"
        ) as config_file:
        ...
        if type(self.proj) == CLIPLastLayer:
            self.clip_model.transformer.resblocks[-2].register_forward_hook(
                self.get_clip_second_last_dense_out
            )

        if pre_trained:
            self.proj.load_state_dict(
                torch.load(os.path.join("weights", f"{proj_name}.pth"), "cpu")
            )
        self.proj.to(device)
        ...
        if self.avg_self_attn_token or self.disentangled_self_attn_token or is_eval:
            self.model.blocks[-1].attn.qkv.register_forward_hook(
    ...
    def forward(self, image, text, return_logit_scale=False):
        ...
        if type(self.proj) == CLIPLastLayer:
            ...
            img_embed, txt_embed = self.proj(
                img_embed, txt_embed, ret_embeds=True, self_attn_maps=self_attn_maps
            )
    ...
    def build_text_embedding(self, text):
        ...
        text_argmax = text.argmax(dim=-1)
        text_argmax = rearrange(
            text_argmax, "n t -> (n t)", n=num_classes, t=num_templates
        )
        ...
        if type(self.proj) == CLIPLastLayer:
            text_embs = torch.cat(
                [
                    self.proj.project_clip_txt(
                        self.encode_text(text[i : i + chunk_size]).permute(1, 0, 2),
                        text_argmax=text_argmax[i : i + chunk_size],
                    )
            ...
            if not self.use_avg_text_token:
                ...
                for i in range(0, N, chunk_size):
                    ...
                    text_embs.append(
                        average_text_tokens(
                            self.feats["clip_txt_out_tokens"]
                            @ self.clip_model.text_projection,
                            text[i : i + chunk_size] > 0,
                            self.keep_cls,
                            self.keep_end_seq,
                        )
                    )
        ...
        # [N, T, C]
        text_embs = rearrange(
            text_embs, "(n t) c -> n t c", n=num_classes, t=num_templates
        )
        # [N, C]
    ...
    def generate_masks(
        ...
        if "dinov2" in self.model_name:
            image_feat = self.model.forward_features(img_preprocessed)[
                "x_norm_patchtokens"
            ]
        elif (
            "mae" in self.model_name
            or "clip" in self.model_name
            or "dino" in self.model_name
        ):
            ...
            image_feat = feats["vit_out"].reshape(
                feats["vit_out"].shape[0],
                feats["vit_out"].shape[1] ** 2,
                feats["vit_out"].shape[-1],
            )  # BS x N_PATCHES x EMBED_DIM

        batch_size, num_tokens, embed_dim = image_feat.shape  # [1, 37*37, 768]
        ...
        mask, simmap = self.masker.forward_seg(
            image_feat, text_emb, hard=False
        )  # [B, N, H', W']

        if self.with_bg_clean:
            mask = self.similarity_assignment_weighted(
                mask, image_feat, self_attn_maps, text_emb, lambda_bg
            )

        # resize
        mask = F.interpolate(
            mask, (pH, pW), mode="bilinear", align_corners=True
        )  # [B, N, H, W]

        if False:
            import cv2

            a = image.detach().cpu().numpy()[0].transpose(1, 2, 0)
            cv2.imwrite("z_img.png", a[:, :, ::-1])
            a = torch.argmax(mask[0], 0).detach().cpu().numpy() / 18 * 255
        ...
        assert (
            mask.shape[2] == H and mask.shape[3] == W
        ), f"shape mismatch: ({H}, {W}) / {mask.shape}"

        return mask, simmap

    def similarity_assignment_weighted(
        self, mask, image_feat, self_attn_maps, text_emb, lambda_bg=0.2
    ):
        ...
        avg_head_embed = (
            self_attn_maps.unsqueeze(2).softmax(dim=-1) * image_feat.unsqueeze(1)
        ).sum(
            dim=-1
        )  # [B, C, M]
        ...
        avg_self_attn_per_class = (self_attn_maps_repeat * head_text_sim_repeat).sum(
            dim=2
        ) / head_text_sim_sum.unsqueeze(-1).repeat(1, 1, hw)
        ...
        avg_self_attn_per_class = (
            avg_self_attn_per_class * (mask.max() - mask.min()) + mask.min()
        )
        mask = mask.reshape(num_classes, hw)  # [N, P]
        mask_output = (mask + lambda_bg * avg_self_attn_per_class).reshape(
            bs, num_classes, h, w
        ) / (1 + lambda_bg)
        return mask_output
...
class DINOJSCC(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if "k" in kwargs.keys():
            proj_name = proj_name + f"_k{kwargs['k']}"
        if pre_trained:
            self.proj.load_state_dict(
                torch.load(os.path.join("weights_mainTIP", f"{proj_name}.pth"), "cpu")
            )
        self.proj.to(device)
    ...
    def generate_masks(
        ...
        if type(self.proj) == DINOJSCC_Adapter:  # 调整 fix_snr
            image_feat = self.proj.visual_jscc(
                image_feat, fix_snr=False, channel_type="awgn"
            )
            text_emb = self.proj.text_adapter(text_emb.float())
            # text_emb = F.interpolate(
            #     text_emb.float().unsqueeze(0).unsqueeze(0),
            #     (text_emb.shape[0], image_feat.shape[-1]),
            #     mode="bilinear",
            #     align_corners=True,
            # )[0][0]
        ...
        if False:
            ...
            a = torch.argmax(mask[0], 0).detach().cpu().numpy() / 18
            plt.figure(figsize=(3, 3), dpi=300)
            plt.imshow(a, cmap="jet")
            plt.axis("off")
            plt.tight_layout(pad=0)
            plt.savefig("z_mask.png")
            plt.close()
            a = cv2.imread("/media/bohnsix/Talk2DINO/z_mask.png")
            img = image.detach().cpu().numpy()[0].transpose(1, 2, 0)
            a = cv2.resize(a, (img.shape[1], img.shape[0]))
            img = np.hstack((img.astype(np.uint8), a.astype(np.uint8)))
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            cv2.imwrite("z_img.png", img)

        if apply_pamr and False:
            for c in range(0, mask.shape[1], 30):
                mask[:, c : c + 30] = self.apply_pamr(ori_image, mask[:, c : c + 30])
