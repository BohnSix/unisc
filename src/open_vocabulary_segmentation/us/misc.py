...
def multi_gpu_test(model,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        ...
        for pred_qualitative, index in zip(result, batch_indices):
            ...
            # resize ndarray seg_map_gt to 896x448
            seg_map_gt = torch.from_numpy(seg_map_gt).unsqueeze(0).unsqueeze(0)
            seg_map_gt = F.interpolate(seg_map_gt, size=(448, 896), mode='nearest')
            seg_map_gt = seg_map_gt.squeeze(0).squeeze(0).numpy()

            # seg_map_gt[seg_map_gt == 255] = 0
            gt_qualitatives.append(seg_map_gt)

            tmp_gt = seg_map_gt.copy()
            for tmp_inx in np.unique(tmp_gt)[1:-1]:
                tmp_gt_ = np.zeros_like(tmp_gt)
                tmp_gt_[tmp_gt != tmp_inx] = 0
                tmp_gt_[tmp_gt == tmp_inx] = 255
                tmp_pred = pred_qualitative.copy() + 1
                tmp_pred[tmp_pred != tmp_inx] = 0 
                tmp_pred[tmp_pred == tmp_inx] = 255

                cv2.imwrite(f"vis/z_pred_{batch_indices[0]:04d}_{tmp_inx}.png",  np.concatenate([tmp_gt_, tmp_pred], axis=1))
