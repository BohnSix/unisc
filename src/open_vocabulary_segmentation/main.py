...
--eval_base src/open_vocabulary_segmentation/configs/cityscapes/eval_cityscapes_pamr.yml
...
def get_argparser():
    ...
    parser.add_argument(
        "--opts",
        help="Modify config options by adding 'KEY=VALUE' list. ",
        default=None,
        nargs="+",
    )
    ...
    parser.add_argument(
        "--wandb", action="store_true", help="Use W&B to log experiments"
    )
    parser.add_argument(
        "--wandb_name", type=str, help="W&B run name", default="default"
    )
    ...
    parser.add_argument("--channelk", type=int)
...
def train(cfg, args):
    ...
    do_training(
        cfg, model, data_loader_train, optimizer, lr_scheduler, scaler, val_loaders
    )
...
def do_training(
    config, model, data_loader, optimizer, lr_scheduler, scaler, val_loaders
):
    logger = get_logger()
    dist.barrier()
    model.train()
    ...
    for step, samples in enumerate(cyclize(data_loader), config.train.start_step):
        ...
        if step % config.print_freq == 0:
            ...
            if wandb is not None:
                log_stat = {
                    f"iter/train_{n}": m.val for n, m in log_vars_meters.items()
                }
...
def validate_seg(config, seg_config, data_loader, model):
    ...
    if device == "cpu" or dist.get_rank() == 0:
        metric = [
            data_loader.dataset.dataset.evaluate(results, metric="mIoU", logger=logger)
        ]
    else:
        metric = [None]
...
"""
python -m torch.distributed.run src/open_vocabulary_segmentation/main.py --eval --eval_cfg src/open_vocabulary_segmentation/configs/cityscapes/dinojscc_cityscapes_vitb_mlp_infonce.yml --eval_base src/open_vocabulary_segmentation/configs/cityscapes/eval_cityscapes.yml --channelk 12 

python -m torch.distributed.run src/open_vocabulary_segmentation/main.py --eval --eval_cfg src/open_vocabulary_segmentation/configs/ade/dinojscc_ade_vitl_mlp_infonce.yml --eval_base src/open_vocabulary_segmentation/configs/ade/eval_ade.yml --channelk 18
"""
