...
def do_train(
    ...
    lr, ltype, num_epochs, batch_size = (
        train_cfg["lr"],
        train_cfg["ltype"],
    ...
    criterion = ContrastiveLoss(
        model, margin=margin, max_violation=max_violation, ltype=ltype
    )
    ...
    for epoch in range(num_epochs):
        ...
        val_loss = validate(model, val_dataloader, criterion, verbose=verbose)
        val_losses[epoch] = val_loss

        print(
            f"Epoch {epoch}: train_loss={train_losses[epoch]} - val_loss={val_losses[epoch]}"
        )

        torch.save(model.state_dict(), f"{out_path}_e{epoch}.pth")
        print(f"Saved model at {out_path}_e{epoch}.pth\n")
